...
OUTPUT_DIR = Path(r"H:\SonicT_Compression_Test\experiment_v4_output")
...
ASV_CODECS = {
    ...
    # V4 additions:
    # These are applied equally to bona fide and spoof samples.
    # This prevents the model from learning "telephone processing = deepfake".
    "wav8k_narrowband": [
        "-ar", "8000",
        "-ac", "1",
        "-c:a", "pcm_s16le",
...
# V4 strategy:
# 1. ASV bona fide and spoof BOTH receive the same narrowband/telephone transforms.
# 2. Personal genuine calls still improve real-phone domain robustness.
# 3. The difficult challenge deepfake is NEVER included in training.
# 4. Success requires improving deepfake detection without destroying
#    held-out genuine-phone accuracy.
...
print("=" * 78)
print("SONICT F1B V4 - DOMAIN-BALANCED NARROWBAND EXPERIMENT")
print("=" * 78)
...
TEST_DATASET_FILE = (
    OUTPUT_DIR /
    "asv_test_features_v4.csv"
)

FAILED_FILE = (
    OUTPUT_DIR /
    "failed_samples_v4.csv"
)
...
narrowband_subset = test_df[
    test_df["condition"].isin(narrowband_names)
]

if not narrowband_subset.empty:
    nb_X = narrowband_subset[FEATURE_NAMES].values
    nb_y = narrowband_subset["label"].values
    nb_pred = best_model.predict(nb_X)

    nb_acc = accuracy_score(nb_y, nb_pred)
    _, _, nb_f1, _ = precision_recall_fscore_support(
        nb_y,
        nb_pred,
        average="macro",
        zero_division=0,
    ...
    print("\nNARROWBAND / TELEPHONE STRESS SUMMARY")
    print("-------------------------------------")
    print(f"Rows      : {len(narrowband_subset)}")
    print(f"Accuracy  : {nb_acc * 100:.2f}%")
    print(f"Macro F1  : {nb_f1 * 100:.2f}%")
else:
    nb_acc = None
    nb_f1 = None
...
MODEL_FILE = (
    OUTPUT_DIR /
    "codec_robust_experimental_model_v4.pkl"
)

FEATURE_FILE = (
    OUTPUT_DIR /
    "codec_robust_features_v4.pkl"
)
...
PERSONAL_RESULT_FILE = (
    OUTPUT_DIR /
    "heldout_personal_results_v4.csv"
)
    ...
    json.dump(
        {
            ...
            "narrowband_stress_accuracy": (
                None if nb_acc is None else float(nb_acc)
            ),
            "narrowband_stress_macro_f1": (
                None if nb_f1 is None else float(nb_f1)
            ),
...
print("\n" + "=" * 78)
print("F1B V4 EXPERIMENT COMPLETE")
print("=" * 78)
...
if nb_f1 is not None:
    print(
        f"Narrowband stress Macro F1    : "
        f"{nb_f1 * 100:.2f}%"
    )
...
print("\nIMPORTANT:")
print("This is still experimental F1B V4.")
