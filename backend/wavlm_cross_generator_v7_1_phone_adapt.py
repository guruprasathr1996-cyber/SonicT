...
# IMPORTANT:
# V7.1 uses a NEW output folder so V7 baseline files are preserved.
OUTPUT_DIR = Path(
    r"H:\SonicT_Compression_Test\experiment_v7_1_phone_adapt"
)
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

MODEL_PATH = OUTPUT_DIR / "wavlm_cross_generator_v7_1_phone_adapt.pt"
...
PHONE_SPLIT_CSV = OUTPUT_DIR / "phone_speaker_split_v7_1.csv"
PHONE_TRAIN_CACHE = OUTPUT_DIR / "phone_train_features_v7_1.npz"
PHONE_VAL_CACHE = OUTPUT_DIR / "phone_val_features_v7_1.npz"
PHONE_TEST_CACHE = OUTPUT_DIR / "phone_test_features_v7_1.npz"

FAILED_CSV = OUTPUT_DIR / "failed_feature_files_v7_1.csv"
...
print("=" * 78)
print("SONICT F1B V7.1 - WAVLM + PHONE-DOMAIN ADAPTATION")
print("=" * 78)
...
print("\n" + "=" * 78)
print("TRAINING V7.1 WAVLM-EMBEDDING MLP")
print("=" * 78)
...
pd.DataFrame(history).to_csv(
    OUTPUT_DIR / "training_history_v7_1.csv",
    index=False,
)
...
print("\n" + "=" * 78)
print("FINAL MLAAD UNSEEN-GENERATOR TEST - V7.1")
print("=" * 78)
...
test_meta["deepfake_probability_v7_1"] = test_probs
test_meta["prediction_v7_1"] = test_pred

test_meta.to_csv(
    OUTPUT_DIR / "unseen_generator_test_results_v7_1.csv",
    index=False,
)
...
print("\n" + "=" * 78)
print("UNSEEN GENERATOR PERFORMANCE - V7.1")
print("=" * 78)
...
for generator in sorted(
    ...
    detection_rate = float(
        np.mean(
            subset["prediction_v7_1"].values == 1
        )
    )

    mean_prob = float(
        subset["deepfake_probability_v7_1"].mean()
    )

    median_prob = float(
        subset["deepfake_probability_v7_1"].median()
    )
...
generator_df.to_csv(
    OUTPUT_DIR / "unseen_generator_performance_v7_1.csv",
    index=False,
)
...
print("\n" + "=" * 78)
print("FINAL UNSEEN-SPEAKER PERSONAL PHONE TEST - V7.1")
print("=" * 78)
...
with open(
    OUTPUT_DIR / "phone_unseen_speaker_results_v7_1.json",
    "w",
    encoding="utf-8",
) as f:
    json.dump(
...
print("\n" + "=" * 78)
print("DIFFICULT KNOWN DEEPFAKE - UNSEEN CHALLENGE - V7.1")
print("=" * 78)
...
if DIFFICULT_DEEPFAKE.exists():
    ...
    challenge_df.to_csv(
        OUTPUT_DIR / "challenge_window_evidence_v7_1.csv",
        index=False,
    )
...
summary = {
    "version": "F1B_V7_1_WAVLM_PHONE_ADAPT",
    "device": str(DEVICE),
...
with open(
    OUTPUT_DIR / "experiment_results_v7_1.json",
    "w",
    encoding="utf-8",
) as f:
    json.dump(
...
print("\n" + "=" * 78)
print("F1B V7.1 PHONE-DOMAIN ADAPTATION COMPLETE")
print("=" * 78)
...
print("\nIMPORTANT:")
print(
    "V7.1 is still experimental. Do NOT integrate it into "
