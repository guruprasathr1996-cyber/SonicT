...
OUTPUT_DIR = Path(
    r"H:\SonicT_Compression_Test\experiment_v6_output"
)
...
BEST_MODEL_PATH = (
    OUTPUT_DIR / "codec_robust_large_cnn_v6.pt"
)
...
def transform_audio(
    ...
    try:
        ...
        intermediate = output_wav.with_suffix(
...
train_df.to_csv(
    OUTPUT_DIR / "asv_train_split_v6.csv",
    index=False,
)

val_df.to_csv(
    OUTPUT_DIR / "asv_validation_split_v6.csv",
    index=False,
)

test_df.to_csv(
    OUTPUT_DIR / "asv_test_split_v6.csv",
    index=False,
)
...
train_meta.to_csv(
    OUTPUT_DIR / "train_window_manifest_v6.csv",
    index=False,
)

val_meta.to_csv(
    OUTPUT_DIR / "validation_window_manifest_v6.csv",
    index=False,
)

test_meta.to_csv(
    OUTPUT_DIR / "test_window_manifest_v6.csv",
    index=False,
)
...
failed_df.to_csv(
    OUTPUT_DIR / "failed_samples_v6.csv",
    index=False,
)
...
pd.DataFrame(history).to_csv(
    OUTPUT_DIR / "training_history_v6.csv",
    index=False,
)
...
file_df.to_csv(
    OUTPUT_DIR / "asv_file_results_v6.csv",
    index=False,
)
...
condition_df.to_csv(
    OUTPUT_DIR / "per_condition_results_v6.csv",
    index=False,
)
...
with open(
    OUTPUT_DIR
    / "heldout_personal_results_v6.json",
    "w",
    encoding="utf-8",
) as file:
    json.dump(
...
if DIFFICULT_DEEPFAKE.exists():
    ...
    challenge_windows.to_csv(
        OUTPUT_DIR
        / "challenge_window_evidence_v6.csv",
        index=False,
    )
